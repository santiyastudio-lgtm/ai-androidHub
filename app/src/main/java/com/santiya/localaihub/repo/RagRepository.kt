...
import com.santiya.localaihub.models.table_schema.RagSourceType
import com.santiya.localaihub.models.table_schema.RagStatus
import com.santiya.localaihub.neuron_example.NeuronGraph
import com.santiya.localaihub.neuron_example.RetrievalConfidence
import com.santiya.localaihub.neuron_example.RetrievalResult
import com.santiya.localaihub.util.DocumentParser
...
class RagRepository(
    ...
    suspend fun createRagFromChat(
        ...
        description: String,
        chatId: String,
        messages: List<com.santiya.localaihub.models.messages.Messages>,
        graph: NeuronGraph,
        domain: String = "general",
        tags: List<String> = emptyList()
    ): Result<InstalledRag> = withContext(Dispatchers.IO) {
    ...
    suspend fun queryAllLoadedGraphs(
        query: String,
        topK: Int = 5
    ): List<Pair<InstalledRag, List<com.santiya.localaihub.neuron_example.QueryResult>>> = withContext(Dispatchers.IO) {
        val results = mutableListOf<Pair<InstalledRag, List<com.santiya.localaihub.neuron_example.QueryResult>>>()
    ...
    suspend fun queryAllLoadedGraphsWithPipeline(
        ...
        for ((ragId, graph) in snapshot) {
            ...
            if (rag == null) {
                android.util.Log.w("RagRepository", "RAG $ragId in loadedGraphs but not in database вЂ” skipping")
                continue
            }
            if (!rag.isEnabled) {
                android.util.Log.w("RagRepository", "RAG ${rag.name} is disabled вЂ” skipping")
                continue
            }
