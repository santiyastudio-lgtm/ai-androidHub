...
// Centralized animation tokens вЂ” use these instead of inline spring()/tween() calls.
object Motion {

    // iOS ease curve
    private val iosEasing = CubicBezierEasing(0.25f, 0.1f, 0.25f, 1f)

    // Interactive press/toggle feedback вЂ” snappy with slight bounce
    ...
    // State changes вЂ” color, alpha, size
    fun <T> state(): FiniteAnimationSpec<T> = tween(
        durationMillis = 200,
        easing = iosEasing
    )

    // Page/modal entrance вЂ” iOS-style curve
    fun <T> entrance(): FiniteAnimationSpec<T> = tween(
        durationMillis = 350,
        easing = iosEasing
    )

    // Exit вЂ” faster than entrance
