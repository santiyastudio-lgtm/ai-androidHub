package com.santiya.localaihub.vault

import android.content.Context
import android.util.Log
import com.santiya.localaihub.BuildConfig
...
import com.santiya.localaihub.models.vault.ChatData
import com.santiya.localaihub.models.vault.ChatInfo
import com.santiya.localaihub.ui.screen.memory.LogLevel
...
object VaultHelper {
    ...
    private inline fun <T> logOperation(
        ...
        return try {
            ...
            VaultLogger.log(LogLevel.INFO, "VAULT", "вњ“ $operation (${duration}ms)")
            result
        } catch (e: Exception) {
            val duration = System.currentTimeMillis() - startTime
            VaultLogger.log(LogLevel.ERROR, "VAULT", "вњ— $operation failed (${duration}ms): ${e.message}", e.stackTraceToString())
            throw e
        }
    }
        ...
        mutex.withLock {
            if (!initialized) {
                try {
                    ...
                    vault = MemoryVault(
                        ...
                        migrationListener = object : MigrationListener {
                            ...
                            override fun onMigrationComplete() {
                                ...
                                VaultLogger.log(LogLevel.INFO, "MIGRATION", "вњ“ Migration completed successfully")
                            }

                            override fun onMigrationFailed(error: Exception) {
                                Log.e("VaultHelper", "Migration failed", error)
                                VaultLogger.log(LogLevel.CRITICAL, "MIGRATION", "вњ— Migration failed", error.stackTraceToString())
                            }
                        }
                    )
                    ...
                    VaultLogger.log(LogLevel.DEBUG, "CRYPTO", "вњ“ Vault index decrypted and loaded (${initDuration}ms)")

                    initialized = true
                    _isReady.value = true
                    VaultLogger.log(LogLevel.INFO, "INIT", "вњ“ Memory Vault initialized successfully")
                } catch (e: Exception) {
                    e.printStackTrace()
    ...
    suspend fun getMessagesForChat(
        ...
        withVaultRecovery("GET_MESSAGES_FOR_CHAT") {
            ...
            VaultLogger.log(LogLevel.DEBUG, "CRYPTO", "вњ“ Decrypted ${messages.size} messages (${decryptDuration}ms)")
    ...
    suspend fun getAllChats(): List<ChatInfo> = withContext(Dispatchers.IO) {
        withVaultRecovery("GET_ALL_CHATS") {
            ...
            val decryptDuration = System.currentTimeMillis() - decryptStart
            VaultLogger.log(LogLevel.DEBUG, "CRYPTO", "вњ“ Decrypted ${chats.size} chat metadata entries (${decryptDuration}ms)")
