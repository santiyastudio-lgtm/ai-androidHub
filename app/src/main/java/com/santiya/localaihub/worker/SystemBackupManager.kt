...
import com.santiya.localaihub.models.table_schema.MemoryCategory
import com.santiya.localaihub.models.vault.ChatExport
...
class SystemBackupManager(private val context: Context) {
    ...
    enum class EntryType {
        DB_FILE,          // Legacy вЂ” Room DB file (v1/v2 backups)
        VAULT_FILE,       // Legacy вЂ” raw vault files (device-bound encryption, not portable)
        DATASTORE_FILE,
        RAG_FILE,
        AVATAR_FILE,
        CHAT_EXPORT,      // Portable вЂ” decrypted chat JSON, re-encrypted on restore
        MODEL_FILE,       // v2 вЂ” model GGUF/SD/TTS files (relative path under models/)
        MANIFEST,         // v2 вЂ” SHA-256 checksums for validation
        MEMORY_EXPORT,    // v3 вЂ” per-persona memory + KG triples as JSON
        UMS_DIR_FILE      // v3 вЂ” UMS directory files (replaces DB_FILE + VAULT_FILE)
    }
    ...
    suspend fun createBackup(
        ...
        try {
            ...
            // Room database (RAG tables only вЂ” kept for FTS4 search)
            collectDatabaseFile()?.let {
                entries.add(Triple(EntryType.DB_FILE, DB_NAME, it))
                checksums[DB_NAME] = sha256(it)
            }

            // Chat exports вЂ” portable JSON (redundant with UMS dir, but useful for cross-version restore)
    ...
    suspend fun restoreBackup(
        ...
        try {
            ...
            // VAULT_FILE entries are skipped вЂ” they use device-bound Keystore encryption
            ...
            try {
                ...
                // Success вЂ” delete safety snapshot
            ...
            // No rollback needed вЂ” we haven't modified anything yet
    ...
    private suspend fun collectModelFiles(options: BackupOptions): List<Pair<String, ByteArray>> {
        ...
        for (model in models) {
            ...
            if (options.modelIdsToInclude.isNotEmpty() && model.id !in options.modelIdsToInclude) {
                Log.d(TAG, "collectModelFiles: skipping '${model.modelName}' вЂ” not in modelIdsToInclude")
                continue
            }
            if (model.pathType == PathType.CONTENT_URI) {
                Log.d(TAG, "collectModelFiles: skipping '${model.modelName}' вЂ” content:// URI")
                continue
            }

            val modelFile = File(model.modelPath)
            if (!modelFile.exists()) {
                Log.w(TAG, "collectModelFiles: skipping '${model.modelName}' вЂ” file not found at ${model.modelPath}")
                continue
            }
            if (!modelFile.absolutePath.startsWith(modelsDir.absolutePath)) {
                Log.d(TAG, "collectModelFiles: skipping '${model.modelName}' вЂ” path not under modelsDir (${modelFile.absolutePath})")
                continue
            }

            if (modelFile.isDirectory) {
                // Diffusion/TTS: walk directory вЂ” only small config/adapter files
    ...
    private fun restoreEntry(type: EntryType, path: String, data: ByteArray) {
        when (type) {
            ...
            EntryType.VAULT_FILE -> {
                // Skip вЂ” device-bound Keystore encryption, not portable
            }
