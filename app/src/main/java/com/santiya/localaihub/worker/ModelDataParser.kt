...
import com.santiya.localaihub.global.formatNumber
...
class ModelDataParser {
    ...
    suspend fun loadModel(
        ...
        return@withContext when (model.providerType) {
            ...
            ProviderType.RAW_ASSET -> loadImportedAssetModel(model, "Raw asset импортирован. Высокий шанс, что он не запустится локально на этом устройстве.")
            ProviderType.TTS -> loadImportedAssetModel(model, "TTS-ассет импортирован.")
        }
    }
    ...
    suspend fun loadModelFromUri(
        ...
        if (providerType != ProviderType.GGUF) {
            return@withContext loadImportedUriAssetModel(context, uri, modelName, providerType)
        }
        try {
            ...
            if (success) {
                // Engine now owns fd вЂ” do not close it
    ...
    private fun loadImportedUriAssetModel(
        context: Context,
        uri: Uri,
        modelName: String,
        providerType: ProviderType
        ...
        return ModelLoadResult.Success(
            info = ImportedAssetModelInfo(
                providerType = providerType,
                architecture = providerType.name,
                name = modelName,
                description = when (providerType) {
                    ProviderType.ONNX -> "ONNX-ассет импортирован. Локальный запуск зависит от поддержанной архитектуры."
                    ProviderType.TTS_PIPER -> "Piper voice pack импортирован. В этой сборке он может храниться как локальный ассет, даже если runtime недоступен."
                    ProviderType.RAW_ASSET -> "Файл импортирован как raw asset. Высокий шанс, что он не запустится локально."
                    ProviderType.TTS -> "TTS-ассет импортирован."
                    else -> "Импортированный ассет."
                },
                parameters = mapOf(
                    "Storage" to "Content URI",
                    "Size" to formatBytes(fileSize),
                    "Source" to uri.toString()
                ),
                additionalInfo = mapOf(
                    "Runtime Status" to if (providerType == ProviderType.RAW_ASSET) "Not guaranteed" else "Experimental"
                )
            ),
            engine = "ImportedAsset"
        )
    }
    ...
    private suspend fun loadDiffusionModel(
        ...
        try {
            ...
            // Check for required files вЂ” clip uses .mnn if runOnCpu OR useCpuClip (matches DiffusionManager)
    ...
    private fun loadImportedAssetModel(
        model: Model,
        description: String
    ): ModelLoadResult {
        val file = File(model.modelPath)
        val size = if (file.exists()) {
            if (file.isDirectory) file.walkTopDown().sumOf { it.length() } else file.length()
        } else {
            0L
        }
        return ModelLoadResult.Success(
            info = ImportedAssetModelInfo(
                ...
                architecture = model.providerType.name,
                name = model.modelName,
                description = description,
                parameters = mapOf(
                    "Storage" to if (file.isDirectory) "Folder" else "File",
                    "Path" to model.modelPath,
                    "Size" to formatBytes(size)
                ),
                additionalInfo = mapOf(
                    "Runtime Status" to if (model.providerType == ProviderType.RAW_ASSET) "Not guaranteed" else "Experimental"
                )
            ),
            engine = "ImportedAsset"
        )
    ...
    private fun buildDiffusionParametersMap(
        ...
        return buildMap {
            ...
            put("Resolution", "${config.width}Г—${config.height}")
...
data class DiffusionModelInfo(
    ...
    override val additionalInfo: Map<String, String> = buildMap {
        ...
        put("Default Size", "${modelConfig.width}Г—${modelConfig.height}")
...
data class ImportedAssetModelInfo(
    override val providerType: ProviderType,
    override val architecture: String,
    override val name: String,
    override val description: String,
