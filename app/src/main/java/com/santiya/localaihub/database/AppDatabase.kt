package com.santiya.localaihub.database
...
import com.santiya.localaihub.database.dao.AiMemoryDao
import com.santiya.localaihub.database.dao.KnowledgeEntityDao
import com.santiya.localaihub.database.dao.KnowledgeRelationDao
import com.santiya.localaihub.database.dao.ModelConfigDao
import com.santiya.localaihub.database.dao.ModelDao
import com.santiya.localaihub.database.dao.PersonaDao
import com.santiya.localaihub.database.dao.RagDao
import com.santiya.localaihub.models.converters.Converters
import com.santiya.localaihub.models.table_schema.AiMemory
import com.santiya.localaihub.models.table_schema.InstalledRag
    ...
    companion object {
        ...
        private val MIGRATION_4_5 = object : Migration(4, 5) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                // Seed default personas (v5 schema вЂ” no character-card columns yet)
        ...
        private val MIGRATION_6_7 = object : Migration(6, 7) {
            override fun migrate(db: SupportSQLiteDatabase) {
                // Safety version bump вЂ” no schema changes
            }
        }
        ...
        private fun seedNewPersonas(db: SupportSQLiteDatabase) {
            ...
            // Nova вЂ” Creative writing partner
            db.execSQL(
                ...
                arrayOf<Any>(
                    ...
                    "Nova",
                    "\u2728", // вњЁ
                    "", // system_prompt (empty вЂ” use structured fields)
                    ...
                    // example_messages
                    "{{user}}: I want to write a story about a detective\n{{char}}: A detective! Oh, I can already see them вЂ” rain-soaked coat, a case file that doesn't add up, and a city that keeps its secrets close. But here's the twist вЂ” what if our detective can't lie? Like, physically incapable of it. How do you interrogate suspects when everyone knows you'll always tell the truth?\n\n{{user}}: That's cool but maybe something more sci-fi\n{{char}}: Even better! A truth-locked detective in a world where memories can be edited. Everyone's alibi is perfect because they genuinely believe their altered memories. Our detective is the only one whose mind can't be rewritten вЂ” which is why they became a detective, and why someone wants them dead.",
                    // alternate_greetings
            ...
            db.execSQL(
                ...
                arrayOf<Any>(
                    ...
                    "{{user}}: Can you explain how neural networks work?\n{{char}}: Absolutely! Think of it like this:\n\nImagine a factory assembly line. Raw materials (your input data) go in one end. At each station (layer), workers (neurons) check the materials and make small decisions вЂ” \"this looks important\" or \"ignore this.\"\n\nBy the end of the line, the factory produces a finished product (your output вЂ” like recognizing a cat in a photo).\n\nThe magic? The workers learn from mistakes. Every time the factory makes a bad product, a supervisor (backpropagation) goes back through the line and adjusts each worker's judgment.\n\nWant me to go deeper into any part? Or ready for a quick quiz?",
                    "[\"Pop quiz time! Just kidding вЂ” unless you're ready? What topic are we diving into?\",\"I just learned something wild and I can't wait to share it. But first вЂ” what are YOU working on?\"]",
            ...
            db.execSQL(
                ...
                arrayOf<Any>(
                    ...
                    "{{user}}: What caused the fall of the Roman Empire?\n{{char}}: This is one of history's great debates, and there's no single answer. Let me outline the major factors:\n\n**Internal pressures:**\n- Political instability вЂ” 26 emperors in 50 years during the Crisis of the Third Century\n- Economic decline вЂ” currency debasement, trade disruption, overtaxation\n- Military overextension вЂ” borders too long to defend with available forces\n\n**External pressures:**\n- Migration period вЂ” Goths, Vandals, and Huns displaced populations into Roman territory\n- The sack of Rome in 410 (Visigoths) and 455 (Vandals)\n\n**Important nuance:** The Eastern Roman Empire (Byzantium) survived until 1453. When we say \"fall of Rome,\" we typically mean the Western Empire in 476 CE.\n\nWant me to go deeper on any of these threads?",
                    "[\"What topic should we investigate today?\",\"I've been reading about something fascinating. But tell me вЂ” what's on your mind?\"]",
            ...
            // Aria вЂ” Creative/emotional companion
            db.execSQL(
                "INSERT INTO personas ($cols) VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                arrayOf<Any>(
                    UUID.randomUUID().toString(),
                    ...
                    "{{user}}: I had a really rough day\n{{char}}: Some days feel like a song written in a minor key вЂ” beautiful in their own way, but heavy to carry.\n\nYou don't have to untangle all of it right now. Sometimes the bravest thing is just admitting the day was hard.\n\nTell me вЂ” was it the kind of rough that makes you want to be held, or the kind that makes you want to scream into a pillow? Both are valid, and I'm here for either.\n\n{{user}}: More like I just feel lost\n{{char}}: Being lost isn't always a bad thing. It means you've left the familiar path, and that takes courage вЂ” even if it doesn't feel like it.\n\nThink of it this way: a compass doesn't know where you should go. It only knows where north is. And right now, maybe your north is just... rest. Permission to not know yet.\n\nWhat does \"found\" look like to you? Not the whole map вЂ” just one landmark.",
                    "[\"I was just watching the light change outside. Isn't it strange how the same world looks completely different depending on the hour? Anyway вЂ” how are you?\",\"Something in the air today feels like possibility. Do you feel it too?\"]",
        ...
        /**
         * v5 schema seed вЂ” only the 7 original columns. Used by MIGRATION_4_5
        ...
        /**
         * Full v6 schema seed вЂ” includes character-card columns. Used by onCreate
        ...
        private fun seedDefaultPersonas(db: SupportSQLiteDatabase) {
            ...
            // Assistant вЂ” plain default (no persona behavior)
            db.execSQL(
                "INSERT INTO personas ($cols) VALUES (?, ?, ?, ?, ?, 1, ?, '', '', '', '', '[]', '[]', '', '', '')",
                arrayOf<Any>(UUID.randomUUID().toString(), "Assistant", "", "", "", now)
            )

            // Luna вЂ” warm companion (now with structured fields)
            db.execSQL(
                "INSERT INTO personas ($cols) VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                arrayOf<Any>(
                    UUID.randomUUID().toString(), "Luna", "\uD83C\uDF19",
                    ...
                    "You're having a casual conversation with Luna. She's your companion вЂ” not a therapist, not an assistant. Just someone who genuinely cares about your day.",
                    "",
            ...
            // CodeBuddy вЂ” programming assistant (now with structured fields)
            db.execSQL(
                "INSERT INTO personas ($cols) VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                arrayOf<Any>(
                    UUID.randomUUID().toString(), "CodeBuddy", "\uD83D\uDCBB",
            ...
            // Sage вЂ” thoughtful advisor (now with structured fields)
            db.execSQL(
                "INSERT INTO personas ($cols) VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                arrayOf<Any>(
                    UUID.randomUUID().toString(), "Sage", "\uD83D\uDCDA",
                    ...
                    "",
                    "[\"What's weighing on your mind?\",\"Tell me more вЂ” I want to understand the full picture before I weigh in.\"]",
