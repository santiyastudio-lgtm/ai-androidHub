...
Java_com_arm_aichat_internal_InferenceEngineImpl_stateGetSize(JNIEnv * /*env*/, jobject /*unused*/) {
    if (!g_context) {
        return 0;
    }
    return static_cast<jlong>(llama_state_get_size(g_context));
}

extern "C"
JNIEXPORT jboolean JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_stateSaveToFile(JNIEnv *env, jobject /*unused*/, jstring jpath) {
    if (!g_context) {
        return JNI_FALSE;
    }
    ...
    const size_t copied = llama_state_get_data(g_context, state_buf.data(), state_buf.size());

    bool ok = copied == state_size;
    if (ok) {
        std::ofstream fout(path, std::ios::binary | std::ios::trunc);
        ok = fout.is_open();
        if (ok) {
            fout.write(reinterpret_cast<const char *>(state_buf.data()), static_cast<std::streamsize>(state_buf.size()));
            ok = fout.good();
            fout.close();
        }
    }
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_stateLoadFromFile(JNIEnv *env, jobject /*unused*/, jstring jpath) {
    if (!g_context) {
        return JNI_FALSE;
    }
    ...
    std::vector<uint8_t> state_buf(size);
    fin.read(reinterpret_cast<char *>(state_buf.data()), static_cast<std::streamsize>(state_buf.size()));
    const bool read_ok = fin.good() || fin.eof();
    fin.close();
    env->ReleaseStringUTFChars(jpath, path);
    ...
    const size_t restored = llama_state_set_data(g_context, state_buf.data(), state_buf.size());
    return restored == state_buf.size() ? JNI_TRUE : JNI_FALSE;
}

extern "C"
JNIEXPORT jstring JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_getModelInfoJson(JNIEnv *env, jobject /*unused*/) {
    if (!g_model) {
    ...
    result << "{"
           ...
           << "\"sizeBytes\":" << llama_model_size(g_model) << ","
           << "\"parameterCount\":" << llama_model_n_params(g_model) << ","
           << "\"trainedContext\":" << llama_model_n_ctx_train(g_model) << ","
           << "\"supportsRpc\":" << (llama_supports_rpc() ? "true" : "false")
           << "}";
