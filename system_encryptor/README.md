...
Native C++ encryption module for SantiyaLocalAiHub. Provides AES-256-GCM encryption, Ed25519 signing/verification, X25519 key exchange, HKDF key derivation, and license verification вЂ” all backed by BoringSSL and exposed to Kotlin via JNI.
...
```
system_encryptor/
в”њв”Ђв”Ђ build.gradle.kts                  # Reads public key from local.properties, passes to CMake
в”њв”Ђв”Ђ src/main/
в”‚   в”њв”Ђв”Ђ java/.../SystemEncryptor.kt   # Kotlin API (JNI facade)
в”‚   в””в”Ђв”Ђ cpp/
в”‚       в”њв”Ђв”Ђ CMakeLists.txt            # Build config, BoringSSL FetchContent
в”‚       в”њв”Ђв”Ђ system_encryptor.cpp      # JNI bridge (extern "C" functions)
в”‚       в”њв”Ђв”Ђ crypto_engine.h/cpp       # Core crypto: AES-GCM, Ed25519, X25519, HKDF
в”‚       в”њв”Ђв”Ђ memory_guard.h/cpp        # SecureBuffer (mmap/mlock/cleanse), secure_zero
в”‚       в”њв”Ђв”Ђ key_store.h/cpp           # Embedded public key (compile-time define)
в”‚       в””в”Ђв”Ђ license_verifier.h/cpp    # License blob verification
```

### Key flow: `local.properties` в†’ binary

```
local.properties
  tn.signing.public_key=<hex>
    в†’ build.gradle.kts reads it via readLines()
      в†’ passes -DTN_SIGNING_PUBLIC_KEY=<hex> to CMake
        в†’ CMakeLists.txt sets target_compile_definitions
          в†’ key_store.cpp uses #define, parses hex at runtime
            в†’ get_embedded_public_key() returns 32-byte key
```
...
| `memory_guard` | `SecureBuffer` вЂ” allocated via `mmap` with `mlock` to prevent swapping. Wiped with `OPENSSL_cleanse` on destruction. Also provides `secure_zero()` and `secure_compare()`. |
...
| CSPRNG | Random bytes | вЂ” | BoringSSL `RAND_bytes` |
...
- The public key is compiled into the native `.so` binary via C preprocessor define вЂ” not stored as a separate asset.
...
- Native symbols are hidden (`-fvisibility=hidden`) вЂ” only `JNIEXPORT` functions are exported.
- BoringSSL is statically linked, no system OpenSSL dependency.
- Private keys and encryption keys in `local.properties` are gitignored вЂ” never commit them.
...
| `Go not found` from BoringSSL | Already handled вЂ” `-DOPENSSL_NO_ASM=1` skips Go-dependent assembly |
| Stale CMake cache | Delete `system_encryptor/.cxx/` and rebuild |
| IDE red squiggles in C++ | Normal вЂ” clangd doesn't resolve BoringSSL includes. NDK build works fine. |
